  ...
  if (action === "signup") {
    ...
    try {
      const user = await mutateStore((store) => {
        ...
        const created = { id: id("u"), email, displayName, passwordHash: hashPassword(password), city: cleanText(body.city, 80), region: cleanText(body.region, 40), postalCode: cleanText(body.postalCode, 12), neighborhood: cleanText(body.neighborhood, 80), bio: cleanText(body.bio, 400), createdAt: nowIso() };
        store.users.push(created);
        store.events.push({ name: "account_create", at: nowIso(), meta: created.id });
        return created;
      });
      await setSession(user.id);
      return NextResponse.json({ user: toPublicUser(user), email: user.email });
    } catch (err) {
      return NextResponse.json({ error: err instanceof Error ? err.message : "Could not create account." }, { status: 409 });
    }
  }
