...
export async function GET(req: Request) {
  ...
  const listings = store.listings.filter((listing) => listingMatchesQuery(listing, {
    q: url.searchParams.get("q") ?? "",
    category: url.searchParams.get("category") ?? "",
    offerType: url.searchParams.get("offerType") ?? "",
    location: url.searchParams.get("location") ?? "",
    radius: Number(url.searchParams.get("radius") ?? "0"),
    origin: lookupPoint(url.searchParams.get("postal") ?? "", url.searchParams.get("location") ?? ""),
...
export async function POST(req: Request) {
  ...
  const category = cleanText(body.category, 40) as CategorySlug;
  const offerType = cleanText(body.offerType, 12) as OfferType;
  ...
  const city = cleanText(body.city, 80) || user.city;
  const region = cleanText(body.region, 40) || user.region;
  if (!city || !region) return NextResponse.json({ error: "Add a city and state/region for pickup." }, { status: 400 });
  ...
  const listing = await mutateStore((store) => {
    const created = {
      id: id("l"),
      slug: `${slugify(title)}-${id("s").slice(-6)}`,
      sellerId: user.id,
      title,
      description,
      category,
      offerType,
      price: offerType === "sell" ? price : null,
      quantity: cleanText(body.quantity, 40),
      city,
      region,
      postalCode: cleanText(body.postalCode, 12) || user.postalCode,
      neighborhood: cleanText(body.neighborhood, 80) || user.neighborhood,
      pickupArea: cleanText(body.pickupArea, 120) || city,
      photos,
    ...
    store.events.push({ name: "listing_create", at: nowIso(), meta: created.id });
    return created;
  });
